...
class MusicPlayer {
    ...
    updateVolumeIcon() {
        ...
        const svg = this.utils.$('.icon-svg', this.elements.volumeBtn);
        const path = this.utils.$('.icon-svg path', this.elements.volumeBtn);
        if (!svg || !path) return;
    ...
    updateUI() {
        // 플레이/정지 버튼 아이콘
        if (this.elements.playPauseBtn) {
            const svg = this.utils.$('.play-pause-icon', this.elements.playPauseBtn);
            const path = this.utils.$('.play-pause-icon path', this.elements.playPauseBtn);
            if (svg && path) {
                if (this.state.isPlaying) {
                    // 일시정지 아이콘 (두 개의 세로 막대)
                    svg.setAttribute('viewBox', '0 0 320 512');
                    path.setAttribute('d', 'M48 64C21.5 64 0 85.5 0 112V400c0 26.5 21.5 48 48 48s48-21.5 48-48V112c0-26.5-21.5-48-48-48zm192 0c-26.5 0-48 21.5-48 48V400c0 26.5 21.5 48 48 48s48-21.5 48-48V112c0-26.5-21.5-48-48-48z');
                } else {
                    // 재생 아이콘 (삼각형)
